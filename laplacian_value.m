function [LValue] = laplacian_value(i, j, window_size, nrow, ncol,  img, regul)
...
if abs(i0-j0) < window_size && abs(i1 - j1) < window_size
    ...
    minx = max([1, i0 - window_size + 1, j0 - window_size + 1]);
    maxx = min([ncol - window_size + 1, i0, j0]);
    miny = max([1, i1 - window_size + 1, j1 - window_size + 1]);
    maxy = min([nrow - window_size + 1, i1, j1]);
    value = 0;
    for x=minx:maxx
        for y=miny:maxy
